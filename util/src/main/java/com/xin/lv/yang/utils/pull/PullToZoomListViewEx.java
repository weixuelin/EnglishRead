...
public class PullToZoomListViewEx extends PullToZoomBase<ListView> implements AbsListView.OnScrollListener {
    ...
    public Adapter getAdapter() {
        return mRootView.getAdapter();
    }

    public int getLastVisiblePosition() {
        return mRootView.getLastVisiblePosition();
    }


    public void setFootView(View view) {
        if (view != null) {
            mRootView.addFooterView(view);
        }

    }

    public void removeFootView(View view) {
        mRootView.removeFooterView(view);
    }
        ...
        if (scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
            ...
            if (view.getLastVisiblePosition() == (view.getCount() - 1)) {
                ...
                if (onScrollRefresh != null) {
                    onScrollRefresh.onLoading(true);
                }
            } else {
                if (onScrollRefresh != null) {
                    onScrollRefresh.onLoading(false);
                }
            }
            ...
            if (view.getFirstVisiblePosition() == 0) {
                ...
                if (firstVisibleItemView != null && firstVisibleItemView.getTop() == 0) {
                    // 可下拉刷新
                    if (onScrollRefresh != null) {
                        onScrollRefresh.onRefresh(true);
                    }

                } else {
                    if (onScrollRefresh != null) {
                        onScrollRefresh.onRefresh(false);
                    }
                }
            }
    ...
    private OnScrollRefresh onScrollRefresh;

    public void setOnScrollRefresh(OnScrollRefresh onScrollRefresh) {
        this.onScrollRefresh = onScrollRefresh;
    }

    public interface OnScrollRefresh {
